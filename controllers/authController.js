...
const register = async (req, res) => {
    try {
        ...
        if (existingUser) { res.status(409).send('User already exists.'); return; }
...
const login = async (req, res) => {
    try {
        ...
        if (!user) { res.status(401).send('Wrong username!'); return; }

        const isValid = bcrypt.compare(password, user.password);
        if (!isValid) { res.status(401).send('Wrong password!'); return; }
